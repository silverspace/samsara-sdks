...
// CreateClient is the # Introduction
//
...
func NewCreateClient(accessToken string, endMs int64, startMs int64, assetID int64, endMs1 *int64, dataInputID int64, startMs1 *int64, durationMs *int64, endMs2 *int64, driverIDOrExternalID string, groupID *int64, timestamp int64, include string, sequenceID string, endingBefore string, limit *float64, startingAfter string, duration *int64, endTime *int64, endTime1 *int64, startTime *int64, endMs3 int64, startMs2 int64, tagID int64, vehicleIDOrExternalID string) CreateClient {
    return NewCreateClientWithBaseURI(DefaultBaseURI, accessToken, endMs, startMs, assetID, endMs1, dataInputID, startMs1, durationMs, endMs2, driverIDOrExternalID, groupID, timestamp, include, sequenceID, endingBefore, limit, startingAfter, duration, endTime, endTime1, startTime, endMs3, startMs2, tagID, vehicleIDOrExternalID)
}

// NewCreateClientWithBaseURI creates an instance of the CreateClient client.
    func NewCreateClientWithBaseURI(baseURI string, accessToken string, endMs int64, startMs int64, assetID int64, endMs1 *int64, dataInputID int64, startMs1 *int64, durationMs *int64, endMs2 *int64, driverIDOrExternalID string, groupID *int64, timestamp int64, include string, sequenceID string, endingBefore string, limit *float64, startingAfter string, duration *int64, endTime *int64, endTime1 *int64, startTime *int64, endMs3 int64, startMs2 int64, tagID int64, vehicleIDOrExternalID string) CreateClient {
        return CreateClient{ NewWithBaseURI(baseURI, accessToken, endMs, startMs, assetID, endMs1, dataInputID, startMs1, durationMs, endMs2, driverIDOrExternalID, groupID, timestamp, include, sequenceID, endingBefore, limit, startingAfter, duration, endTime, endTime1, startTime, endMs3, startMs2, tagID, vehicleIDOrExternalID)}
    }

// DvirMethod create a new dvir, marking a vehicle or trailer safe or unsafe.
    // Parameters:
        // createDvirParam - DVIR creation body
func (client CreateClient) DvirMethod(ctx context.Context, createDvirParam CreateDvirParamType) (result DvirBaseType, err error) {
    if tracing.IsEnabled() {
        ctx = tracing.StartSpan(ctx, fqdn + "/CreateClient.DvirMethod")
        defer func() {
            sc := -1
            ...
            { TargetValue: createDvirParam,
             Constraints: []validation.Constraint{	{Target: "createDvirParam.InspectionType", Name: validation.Null, Rule: true, Chain: nil },
            	{Target: "createDvirParam.UserEmail", Name: validation.Null, Rule: true, Chain: nil }}}}); err != nil {
            return result, validation.NewError(".CreateClient", "DvirMethod", err.Error())
            }

                req, err := client.DvirMethodPreparer(ctx, createDvirParam)
    if err != nil {
    err = autorest.NewErrorWithError(err, ".CreateClient", "DvirMethod", nil , "Failure preparing request")
    return
    }

            resp, err := client.DvirMethodSender(req)
            if err != nil {
            result.Response = autorest.Response{Response: resp}
            err = autorest.NewErrorWithError(err, ".CreateClient", "DvirMethod", resp, "Failure sending request")
            return
            }

            result, err = client.DvirMethodResponder(resp)
            if err != nil {
            err = autorest.NewErrorWithError(err, ".CreateClient", "DvirMethod", resp, "Failure responding to request")
            }

    return
    }

    // DvirMethodPreparer prepares the DvirMethod request.
    func (client CreateClient) DvirMethodPreparer(ctx context.Context, createDvirParam CreateDvirParamType) (*http.Request, error) {
    ...
    autorest.WithJSON(createDvirParam),
    autorest.WithQueryParameters(queryParameters))
    return preparer.Prepare((&http.Request{}).WithContext(ctx))
    }

    // DvirMethodSender sends the DvirMethod request. The method will close the
    // http.Response Body if it receives an error.
    func (client CreateClient) DvirMethodSender(req *http.Request) (*http.Response, error) {
            return autorest.SendWithSender(client, req,
            autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
            }

// DvirMethodResponder handles the response to the DvirMethod request. The method always
// closes the http.Response Body.
func (client CreateClient) DvirMethodResponder(resp *http.Response) (result DvirBaseType, err error) {
