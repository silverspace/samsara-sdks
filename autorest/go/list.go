...
// ListClient is the # Introduction
//
...
// NewListClientWithBaseURI creates an instance of the ListClient client.
    func NewListClientWithBaseURI(baseURI string, accessToken string, endMs int64, startMs int64, assetID int64, endMs1 *int64, dataInputID int64, startMs1 *int64, durationMs *int64, endMs2 *int64, driverIDOrExternalID string, groupID *int64, timestamp int64, include string, sequenceID string, endingBefore string, limit *float64, startingAfter string, duration *int64, endTime *int64, endTime1 *int64, startTime *int64, endMs3 int64, startMs2 int64, tagID int64, vehicleIDOrExternalID string) ListClient {
        return ListClient{ NewWithBaseURI(baseURI, accessToken, endMs, startMs, assetID, endMs1, dataInputID, startMs1, durationMs, endMs2, driverIDOrExternalID, groupID, timestamp, include, sequenceID, endingBefore, limit, startingAfter, duration, endTime, endTime1, startTime, endMs3, startMs2, tagID, vehicleIDOrExternalID)}
    }

// FleetMethod get list of the vehicles. This method returns a list of the vehicles in the Samsara Cloud and
// information about them.
    // Parameters:
        // groupParam - group ID to query.
func (client ListClient) FleetMethod(ctx context.Context, groupParam BodyModelType) (result FleetOKResponseType, err error) {
    if tracing.IsEnabled() {
        ctx = tracing.StartSpan(ctx, fqdn + "/ListClient.FleetMethod")
        defer func() {
            sc := -1
            ...
            return result, validation.NewError(".ListClient", "FleetMethod", err.Error())
            }

                req, err := client.FleetMethodPreparer(ctx, groupParam)
    if err != nil {
    err = autorest.NewErrorWithError(err, ".ListClient", "FleetMethod", nil , "Failure preparing request")
    return
    }

            resp, err := client.FleetMethodSender(req)
            if err != nil {
            result.Response = autorest.Response{Response: resp}
            err = autorest.NewErrorWithError(err, ".ListClient", "FleetMethod", resp, "Failure sending request")
            return
            }

            result, err = client.FleetMethodResponder(resp)
            if err != nil {
            err = autorest.NewErrorWithError(err, ".ListClient", "FleetMethod", resp, "Failure responding to request")
            }

    return
    }

    // FleetMethodPreparer prepares the FleetMethod request.
    func (client ListClient) FleetMethodPreparer(ctx context.Context, groupParam BodyModelType) (*http.Request, error) {
    ...
    autorest.WithPath("/fleet/list"),
    autorest.WithJSON(groupParam),
    autorest.WithQueryParameters(queryParameters))
    return preparer.Prepare((&http.Request{}).WithContext(ctx))
    }

    // FleetMethodSender sends the FleetMethod request. The method will close the
    // http.Response Body if it receives an error.
    func (client ListClient) FleetMethodSender(req *http.Request) (*http.Response, error) {
            return autorest.SendWithSender(client, req,
            autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
            }

// FleetMethodResponder handles the response to the FleetMethod request. The method always
...
// UsersMethod list all users in the organization.
func (client ListClient) UsersMethod(ctx context.Context) (result ListUserType, err error) {
    if tracing.IsEnabled() {
        ctx = tracing.StartSpan(ctx, fqdn + "/ListClient.UsersMethod")
        defer func() {
            sc := -1
            if result.Response.Response != nil {
                sc = result.Response.Response.StatusCode
        ...
        req, err := client.UsersMethodPreparer(ctx)
    if err != nil {
    err = autorest.NewErrorWithError(err, ".ListClient", "UsersMethod", nil , "Failure preparing request")
    return
    }

            resp, err := client.UsersMethodSender(req)
            if err != nil {
            result.Response = autorest.Response{Response: resp}
            err = autorest.NewErrorWithError(err, ".ListClient", "UsersMethod", resp, "Failure sending request")
            return
            }

            result, err = client.UsersMethodResponder(resp)
            if err != nil {
            err = autorest.NewErrorWithError(err, ".ListClient", "UsersMethod", resp, "Failure responding to request")
            }

    return
    }

    // UsersMethodPreparer prepares the UsersMethod request.
    ...
    // UsersMethodSender sends the UsersMethod request. The method will close the
    // http.Response Body if it receives an error.
...
// UsersMethodResponder handles the response to the UsersMethod request. The method always
// closes the http.Response Body.
