 ...
 */
function _vehicles(vehicleUpdateParam, options, callback) {
  ...
  try {
    ...
    if (vehicleUpdateParam === null || vehicleUpdateParam === undefined) {
      throw new Error('vehicleUpdateParam cannot be null or undefined.');
    }
  } catch (error) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'fleet/set_data';
  let queryParameters = [];
  ...
  try {
    if (vehicleUpdateParam !== null && vehicleUpdateParam !== undefined) {
      let requestModelMapper = new client.models['VehicleUpdateParam']().mapper();
      requestModel = client.serialize(requestModelMapper, vehicleUpdateParam, 'vehicleUpdateParam');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(vehicleUpdateParam, null, 2)}.`);
...
class Update {
   ...
   */
  vehiclesWithHttpOperationResponse(vehicleUpdateParam, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._vehicles(vehicleUpdateParam, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  vehicles(vehicleUpdateParam, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._vehicles(vehicleUpdateParam, options, optionalCallback);
    }
  }
