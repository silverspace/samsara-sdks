/*
 * SamsaraAPILib
 *
 * This file was automatically generated by APIMATIC v2.0 ( https://apimatic.io ).
 */
package com.samsara.api.models;

import java.util.*;
import com.fasterxml.jackson.annotation.JsonGetter;
import com.fasterxml.jackson.annotation.JsonSetter;

public class User 
        extends java.util.Observable
        implements java.io.Serializable {
    private static final long serialVersionUID = 7329621547118305111L;
    private AuthTypeEnum authType;
    private String email;
    private String name;
    private String organizationRoleId;
    private Long id;
    private String organizationRole;
    private List<UserTagRole> tagRoles;
    /** GETTER
     * The authentication type the user uses to authenticate. To use SAML this organization must have a configured SAML integration.
     */
    @JsonGetter("authType")
    public AuthTypeEnum getAuthType ( ) { 
        return this.authType;
    }
    
    /** SETTER
     * The authentication type the user uses to authenticate. To use SAML this organization must have a configured SAML integration.
     */
    @JsonSetter("authType")
    public void setAuthType (AuthTypeEnum value) { 
        this.authType = value;
        notifyObservers(this.authType);
    }
 
    /** GETTER
     * The email address of this user.
     */
    @JsonGetter("email")
    public String getEmail ( ) { 
        return this.email;
    }
    
    /** SETTER
     * The email address of this user.
     */
    @JsonSetter("email")
    public void setEmail (String value) { 
        this.email = value;
        notifyObservers(this.email);
    }
 
    /** GETTER
     * The first and last name of the user.
     */
    @JsonGetter("name")
    public String getName ( ) { 
        return this.name;
    }
    
    /** SETTER
     * The first and last name of the user.
     */
    @JsonSetter("name")
    public void setName (String value) { 
        this.name = value;
        notifyObservers(this.name);
    }
 
    /** GETTER
     * The id of the role the user is assigned to at the organization level. This will be blank for users that only have access to specific tags.
     */
    @JsonGetter("organizationRoleId")
    public String getOrganizationRoleId ( ) { 
        return this.organizationRoleId;
    }
    
    /** SETTER
     * The id of the role the user is assigned to at the organization level. This will be blank for users that only have access to specific tags.
     */
    @JsonSetter("organizationRoleId")
    public void setOrganizationRoleId (String value) { 
        this.organizationRoleId = value;
        notifyObservers(this.organizationRoleId);
    }
 
    /** GETTER
     * The ID of the User record.
     */
    @JsonGetter("id")
    public Long getId ( ) { 
        return this.id;
    }
    
    /** SETTER
     * The ID of the User record.
     */
    @JsonSetter("id")
    public void setId (Long value) { 
        this.id = value;
        notifyObservers(this.id);
    }
 
    /** GETTER
     * The name of the role the user is assigned to at the organization level. This will be blank for users that only have access to specific tags.
     */
    @JsonGetter("organizationRole")
    public String getOrganizationRole ( ) { 
        return this.organizationRole;
    }
    
    /** SETTER
     * The name of the role the user is assigned to at the organization level. This will be blank for users that only have access to specific tags.
     */
    @JsonSetter("organizationRole")
    public void setOrganizationRole (String value) { 
        this.organizationRole = value;
        notifyObservers(this.organizationRole);
    }
 
    /** GETTER
     * The specific tags this user has access to. This will be blank for users that have full access to the organization.
     */
    @JsonGetter("tagRoles")
    public List<UserTagRole> getTagRoles ( ) { 
        return this.tagRoles;
    }
    
    /** SETTER
     * The specific tags this user has access to. This will be blank for users that have full access to the organization.
     */
    @JsonSetter("tagRoles")
    public void setTagRoles (List<UserTagRole> value) { 
        this.tagRoles = value;
        notifyObservers(this.tagRoles);
    }
 
}
