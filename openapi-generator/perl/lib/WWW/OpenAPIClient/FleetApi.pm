...
#
package WWW::OpenAPIClient::FleetApi;
...
sub get_driver_document_types_by_org_id {
    ...
    my $_response_object = $self->{api_client}->deserialize('ARRAY[DocumentType]', $response);
    return $_response_object;
}
...
sub get_driver_documents_by_org_id {
    ...
    my $_response_object = $self->{api_client}->deserialize('ARRAY[Document]', $response);
    return $_response_object;
}
...
# @param int $end_ms Time in Unix epoch milliseconds for the end of the query (cannot exceed 1 hour) (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_vehicle_locations' } = { 
        ...
        params => $params,
        returns => 'ARRAY[FleetVehicleLocation]',
        };
}
...
sub get_vehicle_locations {
    ...
    my $_response_object = $self->{api_client}->deserialize('ARRAY[FleetVehicleLocation]', $response);
    return $_response_object;
}
