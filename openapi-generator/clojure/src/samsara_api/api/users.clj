...
(defn-spec delete-user-by-id-with-http-info any?
  "/users/{userId:[0-9]+}
  Remove a user from the organization."
  [access_token string?, userId int?]
  (check-required-params access_token userId)
  (call-api "/users/{userId}" :delete
            {:path-params   {"userId" userId }
             :header-params {}
...
(defn-spec delete-user-by-id any?
  "/users/{userId:[0-9]+}
  Remove a user from the organization."
  [access_token string?, userId int?]
  (let [res (:data (delete-user-by-id-with-http-info access_token userId))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))


(defn-spec get-user-by-id-with-http-info any?
  "/users/{userId:[0-9]+}
  Get a user."
  [access_token string?, userId int?]
  (check-required-params access_token userId)
...
(defn-spec get-user-by-id user-spec
  "/users/{userId:[0-9]+}
  Get a user."
  [access_token string?, userId int?]
  (let [res (:data (get-user-by-id-with-http-info access_token userId))]
    (if (:decode-models *api-context*)
       (st/decode user-spec res st/string-transformer)
       res)))


(defn-spec list-user-roles-with-http-info any?
  "/user_roles
...
(defn-spec list-user-roles (s/coll-of user-role-spec)
  "/user_roles
  Get all roles in the organization."
  [access_token string?]
  (let [res (:data (list-user-roles-with-http-info access_token))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of user-role-spec) res st/string-transformer)
       res)))
...
(defn-spec list-users (s/coll-of user-spec)
  "/users
  List all users in the organization."
  [access_token string?]
  (let [res (:data (list-users-with-http-info access_token))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of user-spec) res st/string-transformer)
       res)))
