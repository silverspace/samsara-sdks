...
case class User (
                  authType:  Option[UserAuthTypeEnum.UserAuthTypeEnum],
...
object UserAuthTypeEnum extends Enumeration {
  val   default, saml = Value
  type UserAuthTypeEnum = Value
  implicit val format: Format[Value] = Format(Reads.enumNameReads(this), Writes.enumNameWrites[UserAuthTypeEnum.type])
}
