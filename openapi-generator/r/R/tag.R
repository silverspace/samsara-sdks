...
Tag <- R6::R6Class(
  ...
  public = list(
    ...
    `sensors` = NULL,
    `vehicles` = NULL,
    initialize = function(`id`, `name`, `addresses`=NULL, `assets`=NULL, `drivers`=NULL, `groupId`=NULL, `machines`=NULL, `parentTagId`=NULL, `sensors`=NULL, `vehicles`=NULL, ...){
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`addresses`)) {
        TagObject[['addresses']] <-
          sapply(self$`addresses`, function(x) x$toJSON())
      }
    ...
    fromJSON = function(TagJson) {
      ...
      if (!is.null(TagObject$`addresses`)) {
        self$`addresses` <- sapply(TagObject$`addresses`, function(x) {
          addressesObject <- TaggedAddress$new()
          addressesObject$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE))
          addressesObject
        })
      }
      if (!is.null(TagObject$`assets`)) {
        self$`assets` <- sapply(TagObject$`assets`, function(x) {
          assetsObject <- TaggedAsset$new()
          assetsObject$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE))
          assetsObject
        })
      }
      if (!is.null(TagObject$`drivers`)) {
        self$`drivers` <- sapply(TagObject$`drivers`, function(x) {
          driversObject <- TaggedDriver$new()
          driversObject$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE))
          driversObject
        })
      }
      if (!is.null(TagObject$`groupId`)) {
        self$`groupId` <- TagObject$`groupId`
      }
      if (!is.null(TagObject$`id`)) {
        self$`id` <- TagObject$`id`
      }
      if (!is.null(TagObject$`machines`)) {
        self$`machines` <- sapply(TagObject$`machines`, function(x) {
          machinesObject <- TaggedMachine$new()
      ...
      if (!is.null(TagObject$`parentTagId`)) {
        self$`parentTagId` <- TagObject$`parentTagId`
      }
      if (!is.null(TagObject$`sensors`)) {
        self$`sensors` <- sapply(TagObject$`sensors`, function(x) {
          sensorsObject <- TaggedSensor$new()
          sensorsObject$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE))
          sensorsObject
        })
      }
      if (!is.null(TagObject$`vehicles`)) {
        self$`vehicles` <- sapply(TagObject$`vehicles`, function(x) {
          vehiclesObject <- TaggedVehicle$new()
    ...
    toJSONString = function() {
      sprintf(
        ...
        paste(unlist(lapply(self$`addresses`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE))), collapse=","),
