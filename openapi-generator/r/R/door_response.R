...
DoorResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      DoorResponseObject <- list()
      if (!is.null(self$`groupId`)) {
        DoorResponseObject[['groupId']] <-
          self$`groupId`
      }
      if (!is.null(self$`sensors`)) {
        DoorResponseObject[['sensors']] <-
          sapply(self$`sensors`, function(x) x$toJSON())
      }

      DoorResponseObject
    },
    fromJSON = function(DoorResponseJson) {
      DoorResponseObject <- jsonlite::fromJSON(DoorResponseJson)
      if (!is.null(DoorResponseObject$`groupId`)) {
        self$`groupId` <- DoorResponseObject$`groupId`
      }
      if (!is.null(DoorResponseObject$`sensors`)) {
        self$`sensors` <- sapply(DoorResponseObject$`sensors`, function(x) {
          sensorsObject <- DoorResponseSensors$new()
    ...
    fromJSONString = function(DoorResponseJson) {
      DoorResponseObject <- jsonlite::fromJSON(DoorResponseJson)
      self$`groupId` <- DoorResponseObject$`groupId`
      data.frame <- DoorResponseObject$`sensors`
      self$`sensors` <- vector("list", length = nrow(data.frame))
      for (row in 1:nrow(data.frame)) {
          sensors.node <- DoorResponseSensors$new()
