...
DriversResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      DriversResponseObject <- list()
      if (!is.null(self$`drivers`)) {
        DriversResponseObject[['drivers']] <-
          sapply(self$`drivers`, function(x) x$toJSON())
      }

      DriversResponseObject
    },
    fromJSON = function(DriversResponseJson) {
      DriversResponseObject <- jsonlite::fromJSON(DriversResponseJson)
      if (!is.null(DriversResponseObject$`drivers`)) {
        self$`drivers` <- sapply(DriversResponseObject$`drivers`, function(x) {
          driversObject <- Driver$new()
    ...
    fromJSONString = function(DriversResponseJson) {
      DriversResponseObject <- jsonlite::fromJSON(DriversResponseJson)
      data.frame <- DriversResponseObject$`drivers`
      self$`drivers` <- vector("list", length = nrow(data.frame))
      for (row in 1:nrow(data.frame)) {
          drivers.node <- Driver$new()
