...
std::string DriversSummaryResponse_Summaries::toJsonString()
{
	std::stringstream ss;
	ptree pt;
	pt.put("DriverId", m_DriverId);
	pt.put("DistanceMiles", m_DistanceMiles);
	pt.put("DriveMs", m_DriveMs);
	pt.put("ActiveMs", m_ActiveMs);
	pt.put("DriverUsername", m_DriverUsername);
	pt.put("GroupId", m_GroupId);
	pt.put("DriverName", m_DriverName);
	pt.put("OnDutyMs", m_OnDutyMs);
	write_json(ss, pt, false);
	return ss.str();
}

void DriversSummaryResponse_Summaries::fromJsonString(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	m_DriverId = pt.get("DriverId", 0L);
	m_DistanceMiles = pt.get("DistanceMiles", 0.0);
	m_DriveMs = pt.get("DriveMs", 0L);
	m_ActiveMs = pt.get("ActiveMs", 0L);
	m_DriverUsername = pt.get("DriverUsername", "");
	m_GroupId = pt.get("GroupId", 0L);
	m_DriverName = pt.get("DriverName", "");
	m_OnDutyMs = pt.get("OnDutyMs", 0L);
}
...
void DriversSummaryResponse_Summaries::setDriverUsername(std::string value)
{
...
void DriversSummaryResponse_Summaries::setDriverName(std::string value)
{
    m_DriverName = value;
}
