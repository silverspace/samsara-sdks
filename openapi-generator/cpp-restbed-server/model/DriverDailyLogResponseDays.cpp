...
std::string DriverDailyLogResponse_days::toJsonString()
{
	std::stringstream ss;
	ptree pt;
	pt.put("CertifiedAtMs", m_CertifiedAtMs);
	pt.put("EndMs", m_EndMs);
	pt.put("StartMs", m_StartMs);
	pt.put("ActiveHours", m_ActiveHours);
	pt.put("DistanceMiles", m_DistanceMiles);
	pt.put("ActiveMs", m_ActiveMs);
	pt.put("Certified", m_Certified);
	write_json(ss, pt, false);
	return ss.str();
}

void DriverDailyLogResponse_days::fromJsonString(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	m_CertifiedAtMs = pt.get("CertifiedAtMs", 0L);
	m_EndMs = pt.get("EndMs", 0);
	m_StartMs = pt.get("StartMs", 0);
	m_ActiveHours = pt.get("ActiveHours", 0.0);
	m_DistanceMiles = pt.get("DistanceMiles", 0.0);
	m_ActiveMs = pt.get("ActiveMs", 0L);
	m_Certified = pt.get("Certified", false);
}
...
void DriverDailyLogResponse_days::setTrailerIds(std::shared_ptr<Object> value)
{
    m_TrailerIds = value;
}
...
void DriverDailyLogResponse_days::setVehicleIds(std::shared_ptr<Object> value)
{
    m_VehicleIds = value;
}
