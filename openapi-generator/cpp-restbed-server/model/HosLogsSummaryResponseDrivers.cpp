...
std::string HosLogsSummaryResponse_drivers::toJsonString()
{
	std::stringstream ss;
	ptree pt;
	pt.put("TimeUntilBreak", m_TimeUntilBreak);
	pt.put("VehicleName", m_VehicleName);
	pt.put("DrivingInViolationToday", m_DrivingInViolationToday);
	pt.put("DriverId", m_DriverId);
	pt.put("CycleRemaining", m_CycleRemaining);
	pt.put("DriverName", m_DriverName);
	pt.put("DutyStatus", m_DutyStatus);
	pt.put("CycleTomorrow", m_CycleTomorrow);
	pt.put("ShiftDriveRemaining", m_ShiftDriveRemaining);
	pt.put("TimeInCurrentStatus", m_TimeInCurrentStatus);
	pt.put("DrivingInViolationCycle", m_DrivingInViolationCycle);
	pt.put("ShiftRemaining", m_ShiftRemaining);
	write_json(ss, pt, false);
	return ss.str();
}

void HosLogsSummaryResponse_drivers::fromJsonString(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	m_TimeUntilBreak = pt.get("TimeUntilBreak", 0L);
	m_VehicleName = pt.get("VehicleName", "");
	m_DrivingInViolationToday = pt.get("DrivingInViolationToday", 0L);
	m_DriverId = pt.get("DriverId", 0L);
	m_CycleRemaining = pt.get("CycleRemaining", 0L);
	m_DriverName = pt.get("DriverName", "");
	m_DutyStatus = pt.get("DutyStatus", "");
	m_CycleTomorrow = pt.get("CycleTomorrow", 0L);
	m_ShiftDriveRemaining = pt.get("ShiftDriveRemaining", 0L);
	m_TimeInCurrentStatus = pt.get("TimeInCurrentStatus", 0L);
	m_DrivingInViolationCycle = pt.get("DrivingInViolationCycle", 0L);
	m_ShiftRemaining = pt.get("ShiftRemaining", 0L);
}
...
void HosLogsSummaryResponse_drivers::setVehicleName(std::string value)
{
    m_VehicleName = value;
}
...
void HosLogsSummaryResponse_drivers::setDriverName(std::string value)
{
    m_DriverName = value;
}
...
void HosLogsSummaryResponse_drivers::setDutyStatus(std::string value)
{
    m_DutyStatus = value;
}
