...
std::string Inline_object_21::toJsonString()
{
	std::stringstream ss;
	ptree pt;
	pt.put("EndMs", m_EndMs);
	pt.put("FillMissing", m_FillMissing);
	pt.put("GroupId", m_GroupId);
	pt.put("StartMs", m_StartMs);
	pt.put("StepMs", m_StepMs);
	write_json(ss, pt, false);
	return ss.str();
}

void Inline_object_21::fromJsonString(std::string const& jsonString)
{
	std::stringstream ss(jsonString);
	ptree pt;
	read_json(ss,pt);
	m_EndMs = pt.get("EndMs", 0);
	m_FillMissing = pt.get("FillMissing", "withNull");
	m_GroupId = pt.get("GroupId", 0L);
	m_StartMs = pt.get("StartMs", 0);
	m_StepMs = pt.get("StepMs", 0);
}
...
void Inline_object_21::setFillMissing(std::string value)
{
    m_FillMissing = value;
}
...
std::vector<std::shared_ptr<_sensors_history_series>> Inline_object_21::getSeries() const
{
    return m_Series;
}
void Inline_object_21::setSeries(std::vector<std::shared_ptr<_sensors_history_series>> value)
{
    m_Series = value;
}
