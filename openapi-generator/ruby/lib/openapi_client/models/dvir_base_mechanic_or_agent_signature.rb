  ...
  class DvirBaseMechanicOrAgentSignature
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DvirBaseMechanicOrAgentSignature` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DvirBaseMechanicOrAgentSignature`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'mechanic_user_id')
        self.mechanic_user_id = attributes[:'mechanic_user_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          mechanic_user_id == o.mechanic_user_id &&
    ...
    def hash
      [mechanic_user_id, driver_id, name, signed_at, type, email, username].hash
    end
