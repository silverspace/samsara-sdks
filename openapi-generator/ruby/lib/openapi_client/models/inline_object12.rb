  ...
  class InlineObject12
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InlineObject12` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InlineObject12`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'previous_defects_corrected')
        self.previous_defects_corrected = attributes[:'previous_defects_corrected']
      end

      if attributes.key?(:'previous_defects_ignored')
        self.previous_defects_ignored = attributes[:'previous_defects_ignored']
      end
