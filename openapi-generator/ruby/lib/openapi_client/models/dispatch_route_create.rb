  ...
  class DispatchRouteCreate
    ...
    def self.openapi_types
      {
        ...
        :'trailer_id' => :'Integer',
        :'vehicle_id' => :'Integer',
        :'dispatch_jobs' => :'Array<DispatchJobCreate>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DispatchRouteCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DispatchRouteCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'actual_end_ms')
        self.actual_end_ms = attributes[:'actual_end_ms']
      end

      if attributes.key?(:'actual_start_ms')
      ...
      if attributes.key?(:'start_location_lng')
        self.start_location_lng = attributes[:'start_location_lng']
      end

      if attributes.key?(:'start_location_name')
      ...
      if attributes.key?(:'dispatch_jobs')
    ...
    def hash
      [actual_end_ms, actual_start_ms, driver_id, group_id, name, scheduled_end_ms, scheduled_meters, scheduled_start_ms, start_location_address, start_location_address_id, start_location_lat, start_location_lng, start_location_name, trailer_id, vehicle_id, dispatch_jobs].hash
    end
