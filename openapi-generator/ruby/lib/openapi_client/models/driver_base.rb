  ...
  class DriverBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DriverBase` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DriverBase`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [eld_adverse_weather_exemption_enabled, eld_big_day_exemption_enabled, eld_day_start_hour, eld_exempt, eld_exempt_reason, eld_pc_enabled, eld_ym_enabled, external_ids, group_id, license_number, license_state, name, notes, phone, username, vehicle_id].hash
    end
