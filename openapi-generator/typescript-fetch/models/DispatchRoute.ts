...
export function DispatchRouteFromJSON(json: any): DispatchRoute {
    return {
        'dispatchJobs': (json['dispatch_jobs'] as Array<any>).map(DispatchJobFromJSON),
        'id': json['id'],
    };
}

export function DispatchRouteToJSON(value?: DispatchRoute): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'dispatch_jobs': (value.dispatchJobs as Array<any>).map(DispatchJobToJSON),
        'id': value.id,
    };
}
