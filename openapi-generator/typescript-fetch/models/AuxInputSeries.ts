...
export function AuxInputSeriesFromJSON(json: any): AuxInputSeries {
    return {
        'name': json['name'],
        'values': (json['values'] as Array<any>).map(AuxInputFromJSON),
    };
}

export function AuxInputSeriesToJSON(value?: AuxInputSeries): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'name': value.name,
        'values': (value.values as Array<any>).map(AuxInputToJSON),
    };
}
