 ...
 */
case class DriverBase(eldAdverseWeatherExemptionEnabled: Option[Boolean],
...
object DriverBase {
    /**
     * Creates the codec for converting DriverBase from and to JSON.
     */
    implicit val decoder: Decoder[DriverBase] = deriveDecoder
    implicit val encoder: ObjectEncoder[DriverBase] = deriveEncoder
}
