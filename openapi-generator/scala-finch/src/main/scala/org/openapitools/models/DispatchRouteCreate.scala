 ...
 * @param dispatchUnderscorejobs The dispatch jobs to create for this route.
 */
case class DispatchRouteCreate(actualUnderscoreendUnderscorems: Option[Long],
                ...
                dispatchUnderscorejobs: Seq[DispatchJobCreate]
                )

object DispatchRouteCreate {
    /**
     * Creates the codec for converting DispatchRouteCreate from and to JSON.
     */
    implicit val decoder: Decoder[DispatchRouteCreate] = deriveDecoder
    implicit val encoder: ObjectEncoder[DispatchRouteCreate] = deriveEncoder
}
