...
object endpoint {
  ...
  */
  def makeService(da: DataAccessor): Service[Request, Response] = (
          AssetsApi.endpoints(da)  :+:
          DriversApi.endpoints(da)  :+:
          FleetApi.endpoints(da)  :+:
          IndustrialApi.endpoints(da)  :+:
          RoutesApi.endpoints(da)  :+:
          SafetyApi.endpoints(da)  :+:
          SensorsApi.endpoints(da)  :+:
          TagsApi.endpoints(da)  :+:
