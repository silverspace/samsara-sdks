...
{
    {
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("addresses")).as_array() )
        {
            if(item.is_null())
            {
                m_Addresses.push_back( std::shared_ptr<_addresses_addresses>(nullptr) );
            }
            else
            {
                std::shared_ptr<_addresses_addresses> newItem(new _addresses_addresses());
...
std::vector<std::shared_ptr<_addresses_addresses>>& Inline_object::getAddresses()
{
    return m_Addresses;
}

void Inline_object::setAddresses(const std::vector<std::shared_ptr<_addresses_addresses>>& value)
{
    m_Addresses = value;
