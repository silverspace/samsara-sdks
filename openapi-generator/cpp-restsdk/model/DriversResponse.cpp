...
void DriversResponse::validate()
{
    // TODO: implement validation
}

web::json::value DriversResponse::toJson() const
{
...
void DriversResponse::fromJson(const web::json::value& val)
{
    {
        m_Drivers.clear();
        std::vector<web::json::value> jsonArray;
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("drivers")).as_array() )
        {
            if(item.is_null())
            {
                m_Drivers.push_back( std::shared_ptr<Driver>(nullptr) );
            }
            else
            {
                std::shared_ptr<Driver> newItem(new Driver());
...
void DriversResponse::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
void DriversResponse::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
...
std::vector<std::shared_ptr<Driver>>& DriversResponse::getDrivers()
{
    return m_Drivers;
}

void DriversResponse::setDrivers(const std::vector<std::shared_ptr<Driver>>& value)
{
    m_Drivers = value;
    m_DriversIsSet = true;
}

bool DriversResponse::driversIsSet() const
{
    return m_DriversIsSet;
}

void DriversResponse::unsetDrivers()
{
    m_DriversIsSet = false;
}
