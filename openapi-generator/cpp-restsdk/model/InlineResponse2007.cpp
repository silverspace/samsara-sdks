...
void Inline_response_200_7::fromJson(const web::json::value& val)
{
    {
        m_Machines.clear();
        std::vector<web::json::value> jsonArray;
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("machines")).as_array() )
        {
            if(item.is_null())
            {
                m_Machines.push_back( std::shared_ptr<Machine>(nullptr) );
            }
            else
            {
                std::shared_ptr<Machine> newItem(new Machine());
...
std::vector<std::shared_ptr<Machine>>& Inline_response_200_7::getMachines()
{
    return m_Machines;
}

void Inline_response_200_7::setMachines(const std::vector<std::shared_ptr<Machine>>& value)
{
    m_Machines = value;
    m_MachinesIsSet = true;
}

bool Inline_response_200_7::machinesIsSet() const
{
    return m_MachinesIsSet;
}

void Inline_response_200_7::unsetMachines()
{
    m_MachinesIsSet = false;
}
