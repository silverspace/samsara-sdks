...
{
    {
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("assets")).as_array() )
        {
            if(item.is_null())
            {
                m_Assets.push_back( std::shared_ptr<Asset>(nullptr) );
            }
            else
            {
                std::shared_ptr<Asset> newItem(new Asset());
...
std::vector<std::shared_ptr<Asset>>& Inline_response_200::getAssets()
{
    return m_Assets;
}

void Inline_response_200::setAssets(const std::vector<std::shared_ptr<Asset>>& value)
{
    m_Assets = value;
    m_AssetsIsSet = true;
}

bool Inline_response_200::assetsIsSet() const
{
    return m_AssetsIsSet;
}

void Inline_response_200::unsetAssets()
{
    m_AssetsIsSet = false;
}
