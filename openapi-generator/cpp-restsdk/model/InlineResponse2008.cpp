...
void Inline_response_200_8::fromJson(const web::json::value& val)
{
    {
        m_Sensors.clear();
        std::vector<web::json::value> jsonArray;
        ...
        for( auto& item : val.at(utility::conversions::to_string_t("sensors")).as_array() )
        {
            if(item.is_null())
            {
                m_Sensors.push_back( std::shared_ptr<Sensor>(nullptr) );
            }
            else
            {
...
std::vector<std::shared_ptr<Sensor>>& Inline_response_200_8::getSensors()
{
    return m_Sensors;
}

void Inline_response_200_8::setSensors(const std::vector<std::shared_ptr<Sensor>>& value)
{
    m_Sensors = value;
    m_SensorsIsSet = true;
}

bool Inline_response_200_8::sensorsIsSet() const
{
    return m_SensorsIsSet;
}

void Inline_response_200_8::unsetSensors()
{
    m_SensorsIsSet = false;
}
