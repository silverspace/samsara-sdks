...
SafetyApi::SafetyApi(std::shared_ptr<Pistache::Rest::Router> rtr) { 
    router = rtr;
};
...
void SafetyApi::setupRoutes() {
    using namespace Pistache::Rest;

    Routes::Get(*router, base + "/fleet/drivers/:driverId/safety/score", Routes::bind(&SafetyApi::get_driver_safety_score_handler, this));
    Routes::Get(*router, base + "/fleet/vehicles/:vehicleId/safety/harsh_event", Routes::bind(&SafetyApi::get_vehicle_harsh_event_handler, this));
    Routes::Get(*router, base + "/fleet/vehicles/:vehicleId/safety/score", Routes::bind(&SafetyApi::get_vehicle_safety_score_handler, this));

    // Default handler, called when a route is not found
    router->addCustomHandler(Routes::bind(&SafetyApi::safety_api_default_handler, this));
}

void SafetyApi::get_driver_safety_score_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
...
void SafetyApi::get_vehicle_harsh_event_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    // Getting the path params
...
void SafetyApi::get_vehicle_safety_score_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    // Getting the path params
    auto vehicleId = request.param(":vehicleId").as<int64_t>();
...
void SafetyApi::safety_api_default_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
