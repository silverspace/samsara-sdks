...
nlohmann::json Tag::toJson() const
{
...

    {
        ...
        for( const auto& item : m_Addresses )
        {
...
void Tag::fromJson(const nlohmann::json& val)
{
    {
        m_Addresses.clear();
        if(val.find("addresses") != val.end())
        {
            for( const auto& item : val["addresses"] )
            {
                
                if(item.is_null())
                {
                    m_Addresses.push_back( TaggedAddress() );
                }
                else
                {
                    TaggedAddress newItem;
    ...
    }
    {
        ...
        if(val.find("assets") != val.end())
        {
            for( const auto& item : val["assets"] )
            {
                ...
                if(item.is_null())
                {
                    m_Assets.push_back( TaggedAsset() );
                }
                else
                {
                    TaggedAsset newItem;
    ...
    }
    {
        ...
        if(val.find("drivers") != val.end())
        {
            for( const auto& item : val["drivers"] )
            {
                ...
                if(item.is_null())
                {
                    m_Drivers.push_back( TaggedDriver() );
                }
                else
                {
                    TaggedDriver newItem;
                    newItem.fromJson(item);
    ...
    setId(val.at("id"));
    {
        ...
        if(val.find("machines") != val.end())
        {
            for( const auto& item : val["machines"] )
            {
                ...
                if(item.is_null())
                {
                    m_Machines.push_back( TaggedMachine() );
                }
                else
                {
                    TaggedMachine newItem;
                    newItem.fromJson(item);
    ...
    }
    {
        ...
        if(val.find("sensors") != val.end())
        {
            for( const auto& item : val["sensors"] )
            {
                ...
                if(item.is_null())
                {
                    m_Sensors.push_back( TaggedSensor() );
                }
                else
                {
                    TaggedSensor newItem;
                    newItem.fromJson(item);
    ...
    }
    {
        ...
        if(val.find("vehicles") != val.end())
        {
            for( const auto& item : val["vehicles"] )
            {
                ...
                if(item.is_null())
                {
                    m_Vehicles.push_back( TaggedVehicle() );
                }
                else
                {
                    TaggedVehicle newItem;
                    newItem.fromJson(item);
...
std::vector<TaggedAddress>& Tag::getAddresses()
{
    return m_Addresses;
}
...
std::vector<TaggedAsset>& Tag::getAssets()
{
    return m_Assets;
}
...
std::vector<TaggedDriver>& Tag::getDrivers()
{
    return m_Drivers;
}
...
void Tag::setGroupId(int64_t const value)
{
    m_GroupId = value;
    m_GroupIdIsSet = true;
}
...
std::vector<TaggedMachine>& Tag::getMachines()
{
    return m_Machines;
}
...
void Tag::setParentTagId(int64_t const value)
{
    m_ParentTagId = value;
...
std::vector<TaggedSensor>& Tag::getSensors()
{
    return m_Sensors;
}
...
std::vector<TaggedVehicle>& Tag::getVehicles()
{
    return m_Vehicles;
}
