...
feature -- API Access
	...
	all_tags (access_token: STRING_32; group_id: INTEGER_64): detachable INLINE_RESPONSE_200_9
			-- /tags
		...
		do
			...
			elseif attached { INLINE_RESPONSE_200_9 } l_response.data ({ INLINE_RESPONSE_200_9 }) as l_data then
				Result := l_data
			else
	...
	create_tag (access_token: STRING_32; tag_create_params: TAG_CREATE): detachable TAG
			...
			-- argument: access_token Samsara API access token. (required)
			-- 
			-- argument: tag_create_params  (required)
			-- 
			-- 
			-- Result TAG
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(tag_create_params)
			l_path := "/tags"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "access_token", access_token));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { TAG } l_response.data ({ TAG }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	delete_tag_by_id (access_token: STRING_32; tag_id: INTEGER_64)
			...
			-- argument: access_token Samsara API access token. (required)
			-- 
			-- argument: tag_id ID of the tag. (required)
			-- 
			-- 
		require
		local
		...
		do
			...
			l_path := "/tags/{tag_id}"
			l_path.replace_substring_all ("{"+"tag_id"+"}", api_client.url_encode (tag_id.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "access_token", access_token));
	...
	modify_tag_by_id (access_token: STRING_32; tag_id: INTEGER_64; tag_modify_params: TAG_MODIFY): detachable TAG
			...
			-- argument: tag_id ID of the tag. (required)
			-- 
			-- argument: tag_modify_params  (required)
			-- 
			-- 
			-- Result TAG
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(tag_modify_params)
			l_path := "/tags/{tag_id}"
			l_path.replace_substring_all ("{"+"tag_id"+"}", api_client.url_encode (tag_id.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "access_token", access_token));
	...
	update_tag_by_id (access_token: STRING_32; tag_id: INTEGER_64; update_tag_params: TAG_UPDATE): detachable TAG
			...
			-- argument: tag_id ID of the tag. (required)
			-- 
			-- argument: update_tag_params  (required)
			-- 
			-- 
			-- Result TAG
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(update_tag_params)
			l_path := "/tags/{tag_id}"
			l_path.replace_substring_all ("{"+"tag_id"+"}", api_client.url_encode (tag_id.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "access_token", access_token));
