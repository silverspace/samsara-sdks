...
feature -- API Access
	...
	delete_user_by_id (access_token: STRING_32; user_id: INTEGER_64)
			...
			-- 
			-- argument: user_id ID of the user. (required)
			-- 
			-- 
		require
		local
		...
		do
			...
			l_path := "/users/{userId}"
			l_path.replace_substring_all ("{"+"userId"+"}", api_client.url_encode (user_id.out))
	...
	list_user_roles (access_token: STRING_32): detachable LIST [USER_ROLE]
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [USER_ROLE] } l_response.data ({ LIST [USER_ROLE] }) as l_data then
				Result := l_data
			else
	...
	list_users (access_token: STRING_32): detachable LIST [USER]
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { LIST [USER] } l_response.data ({ LIST [USER] }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
