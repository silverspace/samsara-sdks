...
feature -- API Access
	...
	sensors (access_token: STRING_32; group_param: INLINE_OBJECT_23): detachable INLINE_RESPONSE_200_8
		...
		do
			...
			elseif attached { INLINE_RESPONSE_200_8 } l_response.data ({ INLINE_RESPONSE_200_8 }) as l_data then
				Result := l_data
			else
	...
	sensors_cargo (access_token: STRING_32; sensor_param: INLINE_OBJECT_19): detachable CARGO_RESPONSE
			...
			-- argument: access_token Samsara API access token. (required)
			-- 
			-- argument: sensor_param  (required)
			-- 
			-- 
			-- Result CARGO_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			reset_error
			create l_request
			l_request.set_body(sensor_param)
			l_path := "/sensors/cargo"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "access_token", access_token));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { CARGO_RESPONSE } l_response.data ({ CARGO_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	sensors_door (access_token: STRING_32; sensor_param: INLINE_OBJECT_20): detachable DOOR_RESPONSE
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { DOOR_RESPONSE } l_response.data ({ DOOR_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	sensors_history (access_token: STRING_32; history_param: INLINE_OBJECT_21): detachable SENSOR_HISTORY_RESPONSE
			...
			-- 
			-- 
			-- Result SENSOR_HISTORY_RESPONSE
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { SENSOR_HISTORY_RESPONSE } l_response.data ({ SENSOR_HISTORY_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	sensors_humidity (access_token: STRING_32; sensor_param: INLINE_OBJECT_22): detachable HUMIDITY_RESPONSE
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { HUMIDITY_RESPONSE } l_response.data ({ HUMIDITY_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	sensors_temperature (access_token: STRING_32; sensor_param: INLINE_OBJECT_24): detachable TEMPERATURE_RESPONSE
		...
		do
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { TEMPERATURE_RESPONSE } l_response.data ({ TEMPERATURE_RESPONSE }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
