# coding: utf-8

from __future__ import absolute_import
from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from samsara.models.base_model_ import Model
from samsara import util


class DvirBaseMechanicOrAgentSignature(Model):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """

    def __init__(self, driver_id: int=None, email: str=None, mechanic_user_id: int=None, name: str=None, signed_at: int=None, type: str=None, username: str=None):  # noqa: E501
        """DvirBaseMechanicOrAgentSignature - a model defined in Swagger

        :param driver_id: The driver_id of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type driver_id: int
        :param email: The email of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type email: str
        :param mechanic_user_id: The mechanic_user_id of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type mechanic_user_id: int
        :param name: The name of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type name: str
        :param signed_at: The signed_at of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type signed_at: int
        :param type: The type of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type type: str
        :param username: The username of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :type username: str
        """
        self.swagger_types = {
            'driver_id': int,
            'email': str,
            'mechanic_user_id': int,
            'name': str,
            'signed_at': int,
            'type': str,
            'username': str
        }

        self.attribute_map = {
            'driver_id': 'driverId',
            'email': 'email',
            'mechanic_user_id': 'mechanicUserId',
            'name': 'name',
            'signed_at': 'signedAt',
            'type': 'type',
            'username': 'username'
        }

        self._driver_id = driver_id
        self._email = email
        self._mechanic_user_id = mechanic_user_id
        self._name = name
        self._signed_at = signed_at
        self._type = type
        self._username = username

    @classmethod
    def from_dict(cls, dikt) -> 'DvirBaseMechanicOrAgentSignature':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The DvirBase_mechanicOrAgentSignature of this DvirBaseMechanicOrAgentSignature.  # noqa: E501
        :rtype: DvirBaseMechanicOrAgentSignature
        """
        return util.deserialize_model(dikt, cls)

    @property
    def driver_id(self) -> int:
        """Gets the driver_id of this DvirBaseMechanicOrAgentSignature.

        ID of the driver who signed the DVIR. Will not be returned if mechanicUserId is returned.  # noqa: E501

        :return: The driver_id of this DvirBaseMechanicOrAgentSignature.
        :rtype: int
        """
        return self._driver_id

    @driver_id.setter
    def driver_id(self, driver_id: int):
        """Sets the driver_id of this DvirBaseMechanicOrAgentSignature.

        ID of the driver who signed the DVIR. Will not be returned if mechanicUserId is returned.  # noqa: E501

        :param driver_id: The driver_id of this DvirBaseMechanicOrAgentSignature.
        :type driver_id: int
        """

        self._driver_id = driver_id

    @property
    def email(self) -> str:
        """Gets the email of this DvirBaseMechanicOrAgentSignature.

        Email of the  agent|mechanic who signed the DVIR.  # noqa: E501

        :return: The email of this DvirBaseMechanicOrAgentSignature.
        :rtype: str
        """
        return self._email

    @email.setter
    def email(self, email: str):
        """Sets the email of this DvirBaseMechanicOrAgentSignature.

        Email of the  agent|mechanic who signed the DVIR.  # noqa: E501

        :param email: The email of this DvirBaseMechanicOrAgentSignature.
        :type email: str
        """

        self._email = email

    @property
    def mechanic_user_id(self) -> int:
        """Gets the mechanic_user_id of this DvirBaseMechanicOrAgentSignature.

        ID of the mechanic who signed the DVIR. Will not be returned if driverId is returned.  # noqa: E501

        :return: The mechanic_user_id of this DvirBaseMechanicOrAgentSignature.
        :rtype: int
        """
        return self._mechanic_user_id

    @mechanic_user_id.setter
    def mechanic_user_id(self, mechanic_user_id: int):
        """Sets the mechanic_user_id of this DvirBaseMechanicOrAgentSignature.

        ID of the mechanic who signed the DVIR. Will not be returned if driverId is returned.  # noqa: E501

        :param mechanic_user_id: The mechanic_user_id of this DvirBaseMechanicOrAgentSignature.
        :type mechanic_user_id: int
        """

        self._mechanic_user_id = mechanic_user_id

    @property
    def name(self) -> str:
        """Gets the name of this DvirBaseMechanicOrAgentSignature.

        The name of the agent or mechanic who signed the DVIR.  # noqa: E501

        :return: The name of this DvirBaseMechanicOrAgentSignature.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str):
        """Sets the name of this DvirBaseMechanicOrAgentSignature.

        The name of the agent or mechanic who signed the DVIR.  # noqa: E501

        :param name: The name of this DvirBaseMechanicOrAgentSignature.
        :type name: str
        """

        self._name = name

    @property
    def signed_at(self) -> int:
        """Gets the signed_at of this DvirBaseMechanicOrAgentSignature.

        The time in millis when the DVIR was signed  # noqa: E501

        :return: The signed_at of this DvirBaseMechanicOrAgentSignature.
        :rtype: int
        """
        return self._signed_at

    @signed_at.setter
    def signed_at(self, signed_at: int):
        """Sets the signed_at of this DvirBaseMechanicOrAgentSignature.

        The time in millis when the DVIR was signed  # noqa: E501

        :param signed_at: The signed_at of this DvirBaseMechanicOrAgentSignature.
        :type signed_at: int
        """

        self._signed_at = signed_at

    @property
    def type(self) -> str:
        """Gets the type of this DvirBaseMechanicOrAgentSignature.

        Type corresponds to whether the signature corresponds to driver|mechanic.  # noqa: E501

        :return: The type of this DvirBaseMechanicOrAgentSignature.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type: str):
        """Sets the type of this DvirBaseMechanicOrAgentSignature.

        Type corresponds to whether the signature corresponds to driver|mechanic.  # noqa: E501

        :param type: The type of this DvirBaseMechanicOrAgentSignature.
        :type type: str
        """

        self._type = type

    @property
    def username(self) -> str:
        """Gets the username of this DvirBaseMechanicOrAgentSignature.

        Username of the  agent|mechanic who signed the DVIR.  # noqa: E501

        :return: The username of this DvirBaseMechanicOrAgentSignature.
        :rtype: str
        """
        return self._username

    @username.setter
    def username(self, username: str):
        """Sets the username of this DvirBaseMechanicOrAgentSignature.

        Username of the  agent|mechanic who signed the DVIR.  # noqa: E501

        :param username: The username of this DvirBaseMechanicOrAgentSignature.
        :type username: str
        """

        self._username = username
